...
%     \myinput{The board of cryptograms \( \boldsymbol{e} = (e_1, ..., e_{n_\mathrm{e}}) \in \mathbb{E}^{n_\mathrm{e}}) \)}
%     \myinput{\( e_j = (R_j, C_j), j \in \{ 1, ..., n_\mathrm{e} \} \)}
...
%     \myinput{\( e_j = (R_j, C_j), j \in \{ 1, ..., n_\mathrm{e} \} \)}
%     \myinput{The partial decryptions \( \boldsymbol{S_i} = (S_{i, 1}, ..., S_{i, n_\mathrm{e}}) \in E(\mathbb{F}_p)^{n_e}, i \in \boldsymbol{T} \)}
%     \For{\( j \gets 1 \) \KwTo $n_\mathrm{e}$ \KwBy $1$}{
