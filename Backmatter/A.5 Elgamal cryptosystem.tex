...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[framed, node distance=0,
        ...
        %All content
        \node[block, first, keep_left] at (s.south -| s.west) (s_1) {
            $\boldsymbol{Y} \gets \{ \}$, $\boldsymbol{P} \gets \{ \}$, $\boldsymbol{c} \gets \{ \}$
            };
        \node[arrow, towards_right, after_arrow, between={s.center}{t.center}, anchor=south west] at (s_1.south -| s.south) (a_1) {
            \hspace{20pt} invitation
            };
        \node[block, after_arrow, keep_middle] at (a_1.south -| t.center) (t_1) {
            $(x_i, Y_i) \gets \mathsf{KeyGen}()$
            };
        \node[arrow, towards_left, after_arrow, between={s.center}{t.center}, anchor=south east] at (t_1.south -| t.center) (a_2) {
            $Y_i$ \hspace{20pt}
            };
        \node[block, after_arrow, keep_middle] at (a_2.south -| s.center) (s_2) {
            $\boldsymbol{Y} \gets \boldsymbol{Y} \cup \{Y_i\}$
            };
        \node[banner, keep_left, after_arrow, between={s.west}{t.east}] at (s_2.south -| s.west) (b_1) {
            when all $\mathcal{T}_i$ have published $Y_i$
            };
        \node[block, keep_middle, after_arrow] at (b_1.south -| s.center) (s_3) {
            set $t \in [\frac{2}{3}n, ..., n]$
            };
        \node[arrow, towards_right, after_arrow, between={s.center}{t.center}, anchor=south west] at (s_3.south -| s.center) (a_3) {
            \hspace{20pt} $t$, $\boldsymbol{Y} = \{Y_1, ..., Y_n\}$
            };
        \node[block, keep_right, after_arrow] at (a_3.south -| t.east) (t_2) {
        ...
        \node[arrow, towards_left, after_arrow, between={s.center}{t.center}, anchor=south east] at (t_2.south -| t.center) (a_4) {
            $P_{i, k}$, $c_{i, j}$ \hspace{100pt}
            };
        \node[block, keep_left, after_arrow] at (a_4.south -| s.west) (s_4) {
            $\boldsymbol{P} \gets \boldsymbol{P} \cup \{P_{i, k}\}$, with $k \in \{1, ..., t-1\}$ \\
            $\boldsymbol{c} \gets \boldsymbol{c} \cup \{c_{i, j}\}$, with $j \in \{1, ..., n\}$
            };
        \node[banner, keep_left, after_arrow, between={s.west}{t.east}] at (s_4.south -| s.west) (b_2) {
        ...
        \node[block, keep_right, after_arrow] at (a_5.south -| t.east) (t_3) {
        ...
        \node[arrow, towards_left, after_arrow, between={s.center}{t.center}, anchor=south east] at (t_3.south -| t.center) (a_6) {
            validates \hspace{80pt}
            };
        \node[banner, keep_left, after_arrow, between={s.west}{t.east}] at (a_6.south -| s.west) (b_3) {
            when all $\mathcal{T}_i$ have validated
            };
        \node[block, keep_middle, after_arrow] at (b_3.south -| s.center) (s_5) {
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[framed, node distance=0,
        ...
        %All content
        \node[block, keep_middle, first] at (s_ik.south -| s.center) (s_1) {
            $\tau \gets \{\}$
            };
        \node[arrow, towards_right, after_arrow, between={s.center}{t.center}, anchor=south west] at (s_1.south -| s.center) (a_1) {
            \hspace{10pt} $e = (R, C)$
            };
        \node[block, keep_middle, after_arrow] at (a_1.south -| t.center) (t_1) {
            $S_i \gets [sx_i]R$ \\
        ...
        \node[block, keep_left, after_arrow] at (a_2.south -| s.west) (s_2) {
        ...
        \node[block, keep_left, after_arrow] at (b_1.south -| s.west) (s_3) {
