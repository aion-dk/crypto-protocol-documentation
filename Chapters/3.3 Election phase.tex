...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[framed,
        ...
        %All content
        \node[process_v2, keep_left, first] at (v_ik.south west) (v_1) {
        ...
        \node[banner_v2, keep_left, after_arrow, between={v.west}{dbb.east}] at (v_1.south west) (b_1) {
            perform protocol from \cref{fig: protocol for writing an item on the bulletin board} to write $b_\mathrm{vec}$ as the $k^\mathrm{th}$ item \\
            $(b_\mathrm{vec}, h_{k-1}, \rho_k) \gets \mathtt{WriteOnBoard}(\mathcal{V}_i, m_\mathrm{vec}, c_\mathrm{vec}, p_\mathrm{vec})$, where \\
            $b_\mathrm{vec} = (m_\mathrm{vec}, c_\mathrm{vec}, \mathcal{V}_i, \sigma_\mathrm{vec}, t_k, p_\mathrm{vec}, h_k)$
            };
        \node[process_v2, keep_right, after_arrow] at (b_1.south east) (dbb_1) {
            ...
            \scriptsize perform protocol from \cref{fig: protocol for writing an item on the bulletin board} to write $b_\mathrm{sec}$ as \\
            \scriptsize the $(k+1)^\mathrm{th}$ item \\
        ...
        \node[arrow_v2, towards_left, between={v.center}{dbb.center}, anchor=north west] at (dbb_1.south -| v.center) (a_1) {
            $b_\mathrm{sec} = (m_\mathrm{sec}, c_\mathrm{sec}, \mathcal{D}, \sigma_\mathrm{sec}, t_{k+1}, p_\mathrm{sec}, h_{k+1})$, \\
            $\rho_{k+1}$, $e_\mathrm{d}$
            };
        \node[process_v2, keep_left, after_arrow] at (a_1.south -| v.west) (v_2) {
            verify that $m_\mathrm{sec} =$ "server encryption commitment", \\
            $h_{k+1} = \mathcal{H}(m_\mathrm{sec} || c_\mathrm{sec} || p_\mathrm{sec} || h_k || t_\mathrm{sec})$, \\
            $\mathsf{SigVer} (Y_\mathcal{D}, \sigma_\mathrm{sec}; m_\mathrm{sec} || c_\mathrm{sec} || p_\mathrm{sec})$ and \\
            $\mathsf{SigVer} (Y_\mathcal{D}, \rho_{k+1}; \sigma_\mathrm{sec} || h_{k+1})$ \\ [7pt]
        ...
        \node[arrow_v2, towards_right, between={v.center}{dbb.center}] at (v_2.south -| v.center) (a_2) {
            $e = (R, C)$, $PK$
            };
        \node[process_v2, keep_right, after_arrow] at (a_2.south -| dbb.east) (dbb_2) {
            verify that $\mathsf{DLVer}(PK, G; R - [r_\mathrm{d}]G)$
            };
        \node[banner_v2, keep_left, after_arrow, between={v.west}{dbb.east}] at (dbb_2.south -| v.west) (b_2) {
            perform protocol from \cref{fig: protocol for writing an item on the bulletin board} to write $b_\mathrm{bc}$ as the $(k+2)^\mathrm{th}$ item \\
            $(b_\mathrm{bc}, h_{k+1}, \rho_{k+2}) \gets \mathtt{WriteOnBoard}(\mathcal{V}_i, m_\mathrm{bc}, c_\mathrm{bc}, p_\mathrm{bc})$
            };
        ...
        \draw[dotted] (v_1.south -| v.center)--(b_1.north -| v.center);
        \draw[dotted] (b_1.south -| v.center)--(v_2.north -| v.center);
        \draw[dotted] (v_2.south -| v.center)--(b_2.north -| v.center);

        \draw[dotted] (b_1.south -| dbb.center)--(dbb_1.north -| dbb.center);
        \draw[dotted] (dbb_1.south -| a_1.east)--(a_1.south east);
        \draw[dotted] (a_2.south east)--(dbb_2.north -| dbb.center);
        \draw[dotted] (dbb_2.south -| dbb.center)--(b_2.north -| dbb.center);
        }
    \end{tikzpicture}
    \caption{Encrypted ballot submission protocol}
    \label{fig: ncrypted ballot submission protocol}
\end{figure}
