...
The \textit{Voter} $\mathcal{V}_i$ follows the protocol in \Cref{fig: voter authentication protocol} to get authorized to vote and the protocol from \Cref{fig: protocol for registering a voter session} to crate a voter session. Specifically, the \textit{Voter} must fulfill the steps:
\begin{enumerate}
    ...
    \item On successful authentication, each \textit{Identity Provider} $\mathcal{I}_j$ generates an identity token $\sigma_{\mathrm{id}, j} \leftarrow \mathsf{Sign}(x_{\mathcal{I}_j}; \mathcal{V}_i)$ (\Cref{alg: sign}) and shares it with the \textit{Voter}.
    
    \item Then, the \textit{Voter} $\mathcal{V}_i$ generates a key pair \( (x_i, Y_i) \leftarrow \mathsf{KeyGen}() \) (\Cref{alg: key gen}) and forwards all identity tokens \( \{\sigma_{\mathrm{id}, 1}, ..., \sigma_{\mathrm{id}, n_\mathrm{i}}\} \) and the public key $Y_i$ to the \textit{Voter Authorization Service} $\mathcal{A}$.
    
    \item If all identity tokens can be verified by \( \mathsf{SigVer} (Y_{\mathcal{I}_j}, \sigma_{\mathrm{id}, j}; \mathcal{V}_i) \) (\Cref{alg: sig ver}) then the \textit{Voter Authorization Service} $\mathcal{A}$ checks that the $\mathcal{V}_i$ is a eligible voter identity, i.e. \( \mathcal{V}_i \in \boldsymbol{\mathcal{V}} \).
    
    \item If eligible then, the \textit{Voter Authorization Service} $\mathcal{A}$ generates an authorization token $\sigma_\mathrm{auth} \leftarrow \mathsf{Sign}(x_\mathcal{A}; vID || Y_i || \phi)$ (\Cref{alg: sign}), where $vID$ is an internally used id for the \textit{Voter} $\mathcal{V}_i$ and $\phi \leftarrow \mathcal{H}(\sigma_{\mathrm{id}, 1} || ... || \sigma_{\mathrm{id}, n_\mathrm{i}})$ is a fingerprint to all the tokens that confirm the identity of the voter.
    ...
    \item The \textit{Voter} $\mathcal{V}_i$ submits the token further to the \textit{Digital Ballot Box} $\mathcal{D}$, which verifies the token by $\mathsf{SigVer} (Y_\mathcal{A}, \sigma_\mathrm{auth}; vID || Y_i || \phi)$ (\Cref{alg: sig ver}) and, if valid, appends a \textit{voter session} item on the bulletin board according to the rules from \Cref{sec: bulletin board event types}. The content of the \textit{voter session} item is extracted from the authorization token $\sigma_\mathrm{auth}$.
...
The \textit{Voter Authorization Service} $\mathcal{A}$ stores a link between the voter identity $\mathcal{V}_i$, all related identity tokens and the authorization token. This is stored for the private auditing process in the post-election phase as described in \Cref{sec: private auditing process}. The link is stored by the \textit{Voter Authorization Service} $\mathcal{A}$ due to the fact that the identity tokens likely contain personal information that must not be disclosed on the public bulletin board.
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[framed, node distance=0,
        ...
        \node[title_3, right=of v] (vas) {
            \textbf{Voter Authorization Service $\mathcal{A}$}};
        \node[title_3, right=of vas] (ip) {
            \textbf{Identity Provider $\mathcal{I}_j$}};
        
        % internal knowledge
        \node[block_3, below=of v] (v_ik) {
            /* internal knowledge: \\
            \phantom{/}* $Y_\mathcal{A}$, $\{Y_{\mathcal{I}_1}, ..., Y_{\mathcal{I}_{n_\mathrm{i}}}\}$ */
            };
        \node[block_3, below=of vas] (vas_ik) {
            /* internal knowledge: \\
            \phantom{/}* $x_\mathcal{A}$, $\boldsymbol{\mathcal{V}}$, $\{ Y_{\mathcal{I}_1}, ..., Y_{\mathcal{I}_{n_\mathrm{i}}} \}$ */
            };
        \node[block_3, below=of ip] (ip_ik) {
            /* internal knowledge: \\
            \phantom{/}* $x_{\mathcal{I}_j}$ */
            };
        
        % All content
        \node[arrow_3_skip, below=of v_ik.south, anchor=north west] (a_1) {
            authenticate as $\mathcal{V}_i$
            };
        \node[block_3, anchor=north west] at (a_1.south -| ip.west) (ip_1) {
            $\sigma_{\mathrm{id}, j} \gets \mathsf{Sign}(x_{\mathcal{I}_j}; \mathcal{V}_i)$
            };
        \node[arrow_3_skip, anchor=north west] at (ip_1.south -| v.south) (a_2) {
            $\sigma_{\mathrm{id}, j}$
            };
        \node[block_3, anchor=north west, text width=140] at (a_2.south -| v.west) (v_1) {
            verify that $\mathsf{SigVer}(Y_{\mathcal{I}_j}, \sigma_{\mathrm{id}, j}; \mathcal{V}_i)$
            };
        \node[banner_3, spaced, anchor=north west] at (v_1.south -| v.west) (b_1) {
            \scriptsize when successfully authenticated with all $\mathcal{I}_j \in \boldsymbol{\mathcal{I}}$ and received $\{\sigma_{\mathrm{id}, 1}, ..., \sigma_{\mathrm{id}, n_\mathrm{i}}\}$
            };
        \node[block_3, spaced, anchor=north west] at (b_1.south -| v.west) (v_2) {
            $(x_i, Y_i) \gets \mathsf{KeyGen}()$
            };
        \node[arrow_3_next, below=of v_2.south, anchor=north west] (a_3) {
            $Y_i, \{\sigma_{\mathrm{id}, 1}, ..., \sigma_{\mathrm{id}, n_\mathrm{i}}\}$
            };
        \node[block_3, anchor=north west, text width=310] at (a_3.south -| vas.west) (vas_1) {
            ...
            generate $vID_i$ based on $\mathcal{V}_i$ \\
            $\phi \gets \mathcal{H}(\sigma_{\mathrm{id}, 1} || ... || \sigma_{\mathrm{id}, n_\mathrm{i}})$ \\
            $\sigma_{\mathrm{auth}, i} \gets \mathsf{Sign}(x_\mathcal{A}, vID_i || Y_i || \phi)$ \\ [6pt]
            /* store internally the tuple: \\
            \phantom{/}* $(\mathcal{V}_i, vID_i, \sigma_{\mathrm{auth}, i}, \{\sigma_{\mathrm{id}, 1}, ..., \sigma_{\mathrm{id}, n_\mathrm{i}}\})$ */
            };
        \node[arrow_3_next, anchor=north west] at (vas_1.south -| v.south) (a_4) {
            $vID_i, \sigma_{\mathrm{auth}, i}$
            };
        \node[block_3, anchor=north west, text width=190] at (a_4.south -| v.west) (v_3) {
            $\phi \gets \mathcal{H}(\sigma_{\mathrm{id}, 1} || ... || \sigma_{\mathrm{id}, n_\mathrm{i}})$ \\
            verify that $\mathsf{SigVer}(Y_\mathcal{A}, \sigma_{\mathrm{auth}, i}, vID_i || Y_i || \phi)$
            };
        
        % Arrows and lines
        \draw[dashed] (v.south west)--(ip.south east);    
        \draw[dashed] (v_ik.south west)--(ip_ik.south east);
        \draw[dotted] (b_1.north west)--(b_1.north east);
        \draw[dotted] (b_1.south west)--(b_1.south east);
        \draw[->] (a_1.south west)--(a_1.south east);
        \draw[->] (a_2.south east)--(a_2.south west);
        \draw[->] (a_3.south west)--(a_3.south east);
        \draw[->] (a_4.south east)--(a_4.south west);
        }
    \end{tikzpicture}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[framed, node distance=0,
        ...
        % internal knowledge
        \node[block, below=of v] (v_ik) {
            /* internal knowledge: \\
            \phantom{/}* $(x_i, Y_i)$, $Y_\mathcal{D}$, $vID_i$, $\phi$, $\sigma_{\mathrm{auth}, i}$ */
            };
        \node[block, below=of dbb] (dbb_ik) {
            /* internal knowledge: \\
            \phantom{/}* $x_\mathcal{D}$, $Y_\mathcal{A}$, $\boldsymbol{h} = \{h_1, ... h_{n_\mathrm{b}}\}$ */
            };
        \node[block, anchor=south west] at (dbb_ik.south -| v_ik.west) (v_ik_corner) {};
        
        %All content
        \node[arrow, below=of v_ik.south, anchor=north west] (a_1) {
            $vID_i, Y_i, \phi, \sigma_{\mathrm{auth}, i}$
            };
        \node[block, anchor=north east] at (a_1.south -| dbb.east) (dbb_1) {
            verify that \\
            $\mathsf{SigVer} (Y_\mathcal{A}, \sigma_{\mathrm{auth}, i}; vID_i || Y_i || \phi)$ \\
            then: \\ [9pt]
            $m \gets$ "voter session" \\
            $c \gets (vID_i, Y_i, \phi, \sigma_{\mathrm{auth}, i})$ \\
            $p \gets$ latest config item from $\boldsymbol{h}$ \\
            $\sigma_\mathcal{D} \gets \mathsf{Sign}(x_\mathcal{D}, m || c || p)$ \\
            $t \gets$ current timestamp \\
            $h_{n_\mathrm{b} + 1} \gets \mathcal{H}(m || c || p || h_{n_\mathrm{b}} || t)$ \\
            $\rho \gets \mathsf{Sign}(x_\mathcal{D}; \sigma_\mathcal{D} || h_{n_\mathrm{b} + 1})$ \\ [9pt]
            /* store on the Bulletin Board \\
            \phantom{/}* as a new item the tuple: \\
            \phantom{/}* $(m, c, \mathcal{D}, \sigma_\mathcal{D}, t, p, h_{n_\mathrm{b} + 1})$ */
            };
        \node[arrow, anchor=north west] at (v.south |- dbb_1.south) (a_2) {
            $p$, $\sigma_\mathcal{D}$, $t$, $h_{n_\mathrm{b}}$, $h_{n_\mathrm{b} + 1}$, $\rho$
            };
        \node[block, below=of a_2.south west] (v_1) {
            $m \gets$ "voter session" \\
            $c \gets (vID_i, Y_i, \phi, \sigma_{\mathrm{auth}, i})$ \\ [9pt]
            verify that \\
            $h_{n_\mathrm{b} + 1} = \mathcal{H}(m || c || p || h_{n_\mathrm{b}} || t)$, \\
            $\mathsf{SigVer} (Y_\mathcal{D}, \sigma_\mathcal{D};  m || c || p)$ and \\
            $\mathsf{SigVer} (Y_\mathcal{D}, \rho;  \sigma_\mathcal{D} || h_{n_\mathrm{b} + 1})$ \\
            };
        
        % Arrows and lines
        \draw[dashed] (v.south west)--(dbb.south east);    
        \draw[dashed] (v_ik_corner.south west)--(dbb_ik.south east);
        \draw[->] (a_1.south west)--(a_1.south east);
        \draw[->] (a_2.south east)--(a_2.south west);
        }
    \end{tikzpicture}
    \caption{Protocol for registering a voter session}
    \label{fig: protocol for registering a voter session}
\end{figure}
...
Depending on the election type (referendum, simple election, multiple choice election, STV election), the text $m$ can be constructed in different ways. We recall from \Cref{app: mapping a message on the elliptic curve} that $m$ can be maximum 30 characters in length. 
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[framed, node distance=0,
        ...
        \node[block, anchor=north west, text width=] at (a2.south -| w2.west) (v3) {
            verify that \\
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[framed, node distance=0,
        ...
        \node[block, anchor=north west] at (a2.south -| w2.west) (v3) {
            verify that \\
