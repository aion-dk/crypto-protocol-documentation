...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[framed,
        ...
        \node[process_v2, keep_middle, after_arrow] at (a_3.south -| va.center) (va_1) {
            ...
            \scriptsize perform protocol \ref{pro: write on board} to write $b_\mathrm{vs}$ as the $k^\mathrm{th}$ item \\ [-2pt]
        ...
        \node[process_v2, keep_left, after_arrow] at (a_4.south -| v.west) (v_3) {
            verify that $b_\mathrm{vs}$ is valid according to validations of protocol \ref{pro: write on board} \\
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[framed,
        ...
        \node[banner_v2, keep_left, after_arrow, between={v.west}{dbb.east}] at (v_1.south west) (b_1) {
            perform protocol \ref{pro: write on board} to write $b_\mathrm{vec}$ as the $k^\mathrm{th}$ item \\
        ...
        \node[process_v2, keep_right, after_arrow] at (b_1.south east) (dbb_1) {
            ...
            \scriptsize perform protocol \ref{pro: write on board} to write $b_\mathrm{sec}$ as the $(k+1)^\mathrm{th}$ item \\ [-2pt]
        ...
        \node[process_v2, keep_left, after_arrow] at (a_1.south -| v.west) (v_2) {
            ...
            according to validations of protocol \ref{pro: write on board}
            };
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[framed,
        ...
        \node[banner_v2, keep_left, after_arrow, between={v.west}{dbb.east}] at (dbb_1.south -| v.west) (b_1) {
            perform protocol \ref{pro: write on board} to write $b_\mathrm{bc}$ as the ${k'}^\mathrm{th}$ item \\
        ...
        \node[process_v2, keep_right, after_arrow] at (b_1.south east) (dbb_2) {
            ...
            \scriptsize perform protocol \ref{pro: write on board} to write $b_\mathrm{vts}$ as the first \\ [-2pt]
...
Additionally, the voting application submits the proof of correct encryption $PK$ to the digitla ballot box, which performs protocol \ref{pro: write on board} if $\mathsf{DLVer}(PK, \{ G \}, \{ R - [r_\mathrm{d}]G \})$ (\cref{alg: dl ver}) succeeds, where the content of the item $c_\mathrm{bc} = e = (R, C)$.
...
Next, the digital ballot box returns to the voting application both items $b_\mathrm{bc}$ and $b_\mathrm{vts}$ together with their respective receipts and previous board hash values, according to the protocol \ref{pro: write on board} described in \cref{sec: writing on the bulletin board}. The voting application performs the validation of the two board items according to the protocol \ref{pro: write on board}.
